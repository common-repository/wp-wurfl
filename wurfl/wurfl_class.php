...
class wurfl_class {
	...
	function wurfl_class($wurfl=Array(), $wurfl_agents=Array()) {
		...
		$this->_toLog('constructor', 'Class Initiated', LOG_DEBUG);
	}
	...
	function _GetFullCapabilities($_id) {
		$this->_toLog('_GetFullCapabilities', "searching for $_id", LOG_DEBUG);
		...
		while ( $_curr_device['fall_back'] != 'generic' && $_curr_device['fall_back'] != 'root' ) {
			...
			$this->_toLog('_GetFullCapabilities', 'parent device:'.$_curr_device['fall_back'].' now going to read its capabilities', LOG_DEBUG);
			$$_curr_device['fall_back'] = $this->_GetDeviceCapabilitiesFromId($_curr_device['fall_back']);
			$_curr_device = $$_curr_device['fall_back'];
		}
		$this->_toLog('_GetFullCapabilities', 'reading capabilities of \'generic\' device', LOG_DEBUG);
...
//echo "scrivo $key=$val<br>\n";
	...
	function _GetDeviceCapabilitiesFromId($_id) {
		$this->_toLog('_GetDeviceCapabilitiesFromId', "reading id:$_id", LOG_DEBUG);
		if ( $_id == 'upgui_generic' ) {
			$this->GUI = true;
		}
		if ( in_array($_id, $this->_wurfl_agents) ) {
			$this->_toLog('_GetDeviceCapabilitiesFromId', 'I have it in wurfl_agents cache, done', LOG_DEBUG);
	...
	function GetDeviceCapabilitiesFromAgent($_user_agent, $_check_accept=false) {
		...
		if ( !stristr($_user_agent, 'MIDP') && !stristr($_user_agent, 'Windows CE') && !stristr($_user_agent, 'PPC') && !stristr($_user_agent, 'Symbian') &&
			...
			|| stristr($_user_agent, 'MSIE 8') ||stristr($_user_agent, 'MSIE 7') || stristr($_user_agent, 'MSIE 6') || stristr($_user_agent, 'MSIE 5') 
			)) {
				
			// This is a web browser. Not even searching
			$this->_toLog('constructor', 'Web browser', LOG_DEBUG);
			$this->browser_is_wap=false;
			...
			if(!$__is){
				$this->_toLog('constructor', 'This browser does not support wml, nor wap, nor xhtml, we will never know if it was an i-mode browser', LOG_DEBUG);
				$this->browser_is_wap=false;
			}
		}
		$this->_toLog('GetDeviceCapabilitiesFromAgent', 'searching for '.$_user_agent, LOG_DEBUG);
		...
		if ( WURFL_USE_CACHE === true ) {
			...
			if ( $this->browser_is_wap === true ) {
				$this->_toLog('GetDeviceCapabilitiesFromAgent', 'Device found in local cache, the id is '.$this->id, LOG_DEBUG);
				if ( count($this->capabilities) == 0 )
					$this->_GetFullCapabilities($this->id);
				else
					$this->_toLog('GetDeviceCapabilitiesFromAgent', 'capabilities found in cache', LOG_DEBUG);
				return true;
			} else if ( count($this->_wurfl) == 0 ) {
				$this->_toLog('GetDeviceCapabilitiesFromAgent', 'cache enabled, WURFL is not loaded, now loading', LOG_DEBUG);
				if ( $this->_cacheIsValid() ) {
					$this->_toLog('GetDeviceCapabilitiesFromAgent', 'loading WURFL from cache', LOG_DEBUG);
					list($cache_stat, $this->_wurfl, $this->_wurfl_agents) = load_cache();
				} else {
					$this->_toLog('GetDeviceCapabilitiesFromAgent', 'loading WURFL from XML', LOG_DEBUG);
					$xml_info = parse();
			...
			if ( count($this->_wurfl) == 0 ) {
				$this->_toLog('GetDeviceCapabilitiesFromAgent', 'WURFL is not loaded, now loading', LOG_DEBUG);
				$xml_info = parse();
				$cache_stat = $xml_info[0];
				$this->_wurfl = $xml_info[1];
				$this->_wurfl_agents = $xml_info[2];
		...
		$this->_toLog('GetDeviceCapabilitiesFromAgent', 'Searching in the agent database for '.$_ua, LOG_DEBUG);
		...
		if ( in_array($_ua, $_wurfl_user_agents) ) {
			...
			$this->_toLog('GetDeviceCapabilitiesFromAgent', 'I found an exact match for '.$_ua.' with id '.$this->id, LOG_DEBUG);
			return true;
		}
	...
	function getDeviceCapability($capability) {
		$this->_toLog('_GetDeviceCapability', 'Searching for '.$capability.' as a capability', LOG_DEBUG);
		...
		foreach ( $deviceCapabilities as $group ) {
			...
			while ( list($key, $value)=each($group) ) {
				if ($key==$capability) {
					$this->_toLog('_GetDeviceCapability', 'I found it, value is '.$value, LOG_DEBUG);
					return $value;
				}
			}
		}
	...
	function _WriteFastAgentToId() {
		...
		foreach($cached_agents as $one) {
			if ( $one['user_agent'] == $_ua ) {
				$this->_toLog('_WriteFastAgentToId', $_ua.' is already cached', LOG_DEBUG);
				return;
			}
		}
...
//		$cached_agents[$new_item_id]['id'] = $this->id; // WURFL unique id
		...
		if ( $new_item_id > MAX_UA_CACHE ) {
			...
			$this->_toLog('_WriteFastAgentToId', 'Cache resized to '.MAX_UA_CACHE.' elements', LOG_DEBUG);
		}
		// store in cache file
		...
		if ( !$fp_cache ) {
			...
			$this->_toLog('_WriteFastAgentToId', 'Created temp file '.$filename.' ', LOG_DEBUG);
		}
		fwrite($fp_cache, "<?php \n");
		...
		*/
		...
		$this->_toLog('_WriteFastAgentToId', 'Done caching user_agent to wurfl_id', LOG_DEBUG);
		return;
	}
	...
	function _ReadFastAgentToId($_ua) {
		...
		foreach ( $cached_agents as $device ) {
			if ( $device['user_agent'] == $_ua ) {
				...
				$this->_toLog('_ReadFastAgentToId', 'Found '.$_ua.' with id='.$device['capabilities']['id'], LOG_DEBUG);
				break;
			}
		}
		return true;
	...
	function _cacheIsValid() {
		...
		if ( $wurfl_stat <= $cache_stat ) {
			...
			$this->_toLog('_cacheIsValid', 'cache file is outdated', LOG_DEBUG);
			return false;
		}
	}
