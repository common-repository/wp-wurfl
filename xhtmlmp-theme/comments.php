...
<?php if ($comments) : ?>
	<h3 id="comments"><?php comments_number(__('No Responses','wp-wurfled'), __('One Response','wp-wurfled'), __('% Responses','wp-wurfled')); print(' '); print(__('to','wp-wurfled')); print(' &#8220;'); the_title(); ?>&#8221;</h3> 
	<ol class="commentlist">
	<?php foreach ($comments as $comment) : ?>
		<li class="<?php echo $oddcomment; ?>" id="comment-<?php comment_ID() ?>">
			<cite><?php comment_author_link() ?></cite> <?php _e('Says:','wp-wurfled'); ?>
...
<?php endif; ?>
<?php if ('open' == $post->comment_status) : ?>
<h3 id="respond"><?php _e('Leave a Reply','wp-wurfled'); ?></h3>
...
<?php if ( $user_ID ) : ?>
<p><?php _e('Logged in as','wp-wurfled'); ?> <a href="<?php echo get_option('siteurl'); ?>/wp-admin/profile.php"><?php echo $user_identity; ?></a>. <a href="<?php echo get_option('siteurl'); ?>/wp-login.php?action=logout" title="<?php _e('Log out of this account','wp-wurfled'); ?>"><?php _e('Logout','wp-wurfled'); ?> &raquo;</a></p>
<?php else : ?>
